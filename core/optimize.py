# core/optimizer.py
# v1.2.0  ASCII-only. Loads saved models, builds response functions, and runs discrete optimizations.
...
# ---------------- Results-root discovery (same policy as other pages) ----------------
def _abs(p: str) -> str:
...
def pick_writable_results_root() -> str:
    ...
    if env_dir:
        candidates.append(_abs(env_dir))
    candidates += [
        ...
        for jf in files:
            ...
            try:
                with open(jf, "r", encoding="utf-8") as f:
                    r = json.load(f)
                # tolerate any saved JSON, only index "model-like"
...
# ---------------- Safe getters from model JSON ----------------
def get_coeffs(model: Dict[str, Any]) -> Dict[str, float]:
    # support "coefficients" or "coef"
...
def get_channels(model: Dict[str, Any]) -> List[str]:
    # try explicit, else keys in coeffs excluding intercept
    ch = model.get("channels") or model.get("features") or list(get_coeffs(model).keys())
    return [c for c in ch if str(c).lower() not in ("intercept", "_intercept", "(intercept)")]  # filter
...
def get_current_spend(model: Dict[str, Any]) -> Dict[str, float]:
    # optional, else zeros (user can edit on UI)
...
def get_impact_shares(model: Dict[str, Any]) -> Dict[str, float]:
    # normalized to sum to ~1 (excluding base if present)
    inc = model.get("impact_shares") or model.get("decomposition") or {}
    # strip any "base"/"intercept"
...
def get_segments(model: Dict[str, Any]) -> List[str]:
    # optional metadata: ["Cardio","Onco",...] etc.
...
def build_channel_response_func(model: Dict[str, Any], channel: str):
    """
    Returns a function f(spend)->effect_units using the model's transform configuration if available.
    Falls back to a generic concave curve if unknown.
    We then scale by the channel's impact share so that relative contributions align with the model.
    """
    ...
    scale = float(shares.get(channel, 1.0))  # relative scaler (keeps ratios sensible)

    if ttype == "log":
        def f(sp): 
            return scale * _log1p(sp, k=k)
    else:
        # treat negexp and negexp_cann the same for curve; cannibalization is not modeled in standalone curves
        def f(sp):
            return scale * _negexp(sp, k=k, beta=beta)
    return f
...
def greedy_max_response(
    ...
    """
    Allocate 'total_budget' across channels to maximize sum f_i(spend_i) using discrete steps.
    Respects per-channel min/max and lock constants.
    """
    ch = list(funcs.keys())
    step = float(max(1e-9, step))
    # initialize
    spend = {c: float(max(min_b.get(c, 0.0), start.get(c, 0.0))) for c in ch}
    # honor locks by snapping to min=max=locked spend (if set)
    if locks:
        for c, is_locked in locks.items():
            if is_locked:
                ...
                min_b[c] = v
                max_b[c] = v
    # compute remaining increments to reach target
    current_total = sum(spend.values())
    target_total = float(total_budget)
    # If current > target, greedily take from the least damaging marginals
    def marginal_gain(c, s):
        return funcs[c](s + step) - funcs[c](s)

    if current_total < target_total:
        # add units until target
        iterations = int((target_total - current_total) / step + 0.5)
        for _ in range(max(0, iterations)):
            best_c = None; best_gain = -1e18
            for c in ch:
                if spend[c] + step <= max_b.get(c, float("inf")) + 1e-12:
                    g = marginal_gain(c, spend[c])
                    if g > best_gain:
                        best_gain = g; best_c = c
            if best_c is None:  # all at max
                break
            spend[best_c] += step
    elif current_total > target_total:
        # remove units where it hurts least (smallest marginal loss)
        iterations = int((current_total - target_total) / step + 0.5)
        for _ in range(max(0, iterations)):
            best_c = None; best_loss = 1e18
            for c in ch:
                if spend[c] - step >= min_b.get(c, 0.0) - 1e-12:
                    loss = funcs[c](spend[c]) - funcs[c](spend[c] - step)
                    if loss < best_loss:
                        best_loss = loss; best_c = c
            if best_c is None:
                break
            spend[best_c] -= step
    # clamp
    for c in ch:
        spend[c] = min(max(spend[c], min_b.get(c, 0.0)), max_b.get(c, float("inf")))
    return spend
...
def greedy_profit_with_floor(
    ...
    """
    Profit objective: allocate increments only where marginal ROI >= mroi_floor (default 1.0).
    Returns spends; if nothing clears the floor, returns mins/locks.
    """
    ch = list(funcs.keys())
    step = float(max(1e-9, step))
    spend = {c: float(max(min_b.get(c, 0.0), start.get(c, 0.0))) for c in ch}
    ...
    improved = True
    # keep adding where mROI >= floor until none left or max constraints hit
    while improved:
        improved = False
        best_c = None; best_mroi = mroi_floor
        for c in ch:
            if spend[c] + step <= max_b.get(c, float("inf")) + 1e-12:
                r = mroi(c, spend[c])
                if r >= mroi_floor and r > best_mroi + 1e-12:
                    best_mroi = r; best_c = c
